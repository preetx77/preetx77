<!-- Profile README for preetx77 -->
<p align="center">
  <img src="https://user-images.githubusercontent.com/74038190/212284100-561aa473-3905-4a80-b561-0d28506553ee.gif" width="100%" alt="Animated Banner"/>
</p>

<h1 align="center">Hey 👋, I'm Preet</h1>
<h3 align="center">📈  Developer | 🤖 NLP Engineer | 🧠 Systems Builder</h3>

<p align="center">
  <img src="https://media.giphy.com/media/hvRJCLFzcasrR4ia7z/giphy.gif" width="35px"/>
  <br/>
  Welcome to my GitHub profile!
</p>

---

## 🌟 About Me

- 📊 Building **quantitative finance tools**  : GARCH volatility models, NLP-driven sentiment forecasters, and sector scoring engines
- 🤖 Integrating **FinBERT + GARCH-X** to model how news sentiment drives gold futures volatility
- 🌐 Also ship full-stack apps — **Flask, Streamlit, React, Flutter** : with real data pipelines underneath

---

## 📊 GitHub Stats

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=preetx77&show_icons=true&theme=tokyonight&hide_border=true" height="180"/>
  <img src="https://streak-stats.demolab.com?user=preetx77&theme=highcontrast&hide_border=true" height="180"/>
</p>

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=preetx77&layout=compact&theme=radical&hide_border=true" height="150"/>
</p>

---

## ⚡ Tech Stack

<p align="center">
  <img src="https://img.shields.io/badge/Python-14354C?style=for-the-badge&logo=python&logoColor=yellow"/>
  <img src="https://img.shields.io/badge/React-20232A?style=for-the-badge&logo=react&logoColor=61DAFB"/>
  <img src="https://img.shields.io/badge/Next.js-000000?style=for-the-badge&logo=next.js&logoColor=white"/>
  <img src="https://img.shields.io/badge/Node.js-43853D?style=for-the-badge&logo=node.js&logoColor=white"/>
  <img src="https://img.shields.io/badge/Flutter-02569B?style=for-the-badge&logo=flutter&logoColor=white"/>
  <img src="https://img.shields.io/badge/Streamlit-FF4B4B?style=for-the-badge&logo=streamlit&logoColor=white"/>
  <img src="https://img.shields.io/badge/Flask-000000?style=for-the-badge&logo=flask&logoColor=white"/>
  <img src="https://img.shields.io/badge/TailwindCSS-38B2AC?style=for-the-badge&logo=tailwind-css&logoColor=white"/>
</p>

---

## 📈 Contribution Activity

[![Preet's GitHub Activity Graph](https://github-readme-activity-graph.vercel.app/graph?username=preetx77&theme=react-dark&hide_border=true)](https://github.com/ashutosh00710/github-readme-activity-graph)

---

## 📌 Featured Projects

| Project | Stack | What it does |
|---|---|---|
| 🥇 [gold-sentiment](https://github.com/preetx77/gold-sentiment) | GARCH-X · FinBERT · Streamlit | Forecasts gold futures volatility using NLP sentiment as exogenous regressor — 21-day ahead output |
| 📊 [sector-scoring](https://github.com/preetx77/sector-scoring) | Python · Pandas · HTML/JS | Multi-factor sector ranking engine across momentum, volatility & relative strength |
| 📉 [Volatility Regime Engine](https://github.com/preetx77/portfolio-manager) | GARCH · NumPy · Matplotlib | Classifies market into Low / Normal / Stress regimes; derives risk-parity rebalancing signals |
| 🌿 [Vedyura WebApp](https://github.com/preetx77/Vedyura-WebApp) | Flask · OpenCV · Firebase | Ayurvedic health platform with camera-based PPG heart rate (Nadi Pariksha) + dosha engine |
| 💼 [Portfolio Tracker](https://github.com/preetx77/portfolio-manager) | Streamlit · SQLite · yfinance | Real-time portfolio management with Sharpe ratio, risk score, AI-powered PDF reports |

---

## 🎨 Extras

### 💻 Profile Views
![Profile Views](https://komarev.com/ghpvc/?username=preetx77&color=blueviolet&style=flat-square)

### ✨ Random Dev Quote
![Quote](https://quotes-github-readme.vercel.app/api?type=horizontal&theme=radical)

---

<h3 align="center">✨ Thanks for visiting ✨</h3>
<p align="center">
  Always open to collaborating on meaningful projects — whether it's <b>quant systems</b>, <b>NLP pipelines</b>, or <b>ideas worth building</b>.
</p>
